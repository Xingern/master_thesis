...
import numpy as np
import pandas as pd
from preprocessing import initial_clean
...
def lysekil():
    ...
    if os.path.exists("data/cached_Lysekil.pkl"):
        raw_df = pd.read_pickle("data/cached_Lysekil.pkl")
    else:
        raw_df = pd.read_excel("data/Lysekil_MEA_data_unprotected.xlsx", 
                            header=None, 
                            names=headers, 
                            usecols=import_cols, 
                            skiprows=skip)
        raw_df.to_pickle("data/cached_Lysekil.pkl")

    df = initial_clean(raw_df)
    return df